...
class DatabaseConnection
  ...
  def self.update(database:, table:, key_column:, primary_key:, values:, columns:)
    sql = "UPDATE #{table} SET #{columns} = #{values} WHERE #{key_column} = #{primary_key}"
    run_sql(database: database, sql: sql)
  end
  
  def self.delete(database:, table:, key_column:, primary_key:)
